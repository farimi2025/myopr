...
$kelas = dbFetch("SELECT * FROM kelas WHERE id=?", [$id]);
if (!$kelas) {
    setFlash('danger', 'Rekod kelas tidak dijumpai.');
    redirect(BASE_URL . '/modules/kelas/index.php');
}

$pageTitle   = 'Edit Kelas - ' . $kelas['nama_kelas'];
$errors      = [];
$old         = $kelas;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf()) {
        ...
        redirect(BASE_URL . '/modules/kelas/edit.php?id=' . $id);
    }
    ...
    if ($namaKelas && $tahun) {
        $cek = dbValue("SELECT id FROM kelas WHERE nama_kelas=? AND tahun=? AND id!=?", [$namaKelas, $tahun, $id]);
    ...
    if (empty($errors)) {
        ...
        logAktiviti('kemaskini', 'kelas', $id, "Kemaskini kelas: $namaKelas ($tahun)");
        setFlash('success', "Kelas <strong>$namaKelas</strong> berjaya dikemaskini.");
        redirect(BASE_URL . '/modules/kelas/index.php');
    }
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1"><i class="bi bi-pencil-square me-2 text-warning"></i>Edit Kelas</h4>
    ...
    <div class="d-flex gap-2">
        <a href="guru_subjek.php?kelas_id=<?= $id ?>" class="btn btn-outline-info btn-sm">
            <i class="bi bi-journals me-1"></i>Urus Guru & Subjek
        </a>
